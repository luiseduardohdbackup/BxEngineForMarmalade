...
CREATE_SCENE_FIRST(TestListClass, "Test-List")
...
void OnUpdate(TestListClass& Data)
{
}

void OnRender(TestListClass& Data, BxDraw& Draw)
{
	Draw.AddStyle(COLOR(160, 160, 192));
	Draw.Rectangle(FILL, XYWH(0, 0, Draw.Width(), Draw.Height()));

	BxCore::Font::SetSort(fontsort_pad5);
	BxCore::Font::Draw(Data.font1, "TEST LIST", XY(0, 0), WH(Draw.Width(), 50), RGB(255, 255, 255), 0xFF);
	...
	for(int i = 0; i < TestListClass::ButtonLength; ++i)
	{
		...
		if(Draw.AddStyleByIf(CLIP(XYWH(PosX, PosY, ButtonWidth, ButtonHeight)), ButtonID[i]))
		{
			...
			BxCore::Font::Draw(Data.font2, ButtonID[i], XY(PosX + ((Data.IsButtonPushed[i])? 1 : 0), PosY + ((Data.IsButtonPushed[i])? 1 : 0)),
				WH(Draw.Width(), Draw.Height()), RGB(0, 0, 0), 192);
			Draw.SubStyle();
		}
	}
	...
	BxCore::Font::Draw(Data.font2, Date, XY(0, Draw.Height() - 10 - 30),
		WH(Draw.Width(), 30), RGB(255, 255, 0), 128);

	if(Draw.AddStyleByIf(CLIP(XYWH(Draw.Width() - 10 - 30, (50 - 30) / 2, 30, 30)), "Exit"))
	{
