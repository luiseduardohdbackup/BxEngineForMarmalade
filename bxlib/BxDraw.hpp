...
namespace BxDrawGlobal
{
	...
	*/
	/// @cond SECTION_NAME
	FUNCTOR_BEGIN(XY, const point, int x, int y)
	{
	...
	*/
	/// @cond SECTION_NAME
	FUNCTOR_BEGIN(WH, const size, int w, int h)
	{
	...
	*/
	/// @cond SECTION_NAME
	FUNCTOR_BEGIN(XYWH, const rect, int x, int y, int w = size::max, int h = size::max)
	{
	...
	*/
	...
	\return 다각영역
	*/
	static inline const points& XYS(uint count, int x, int y, ...)
	{
	...
	\return 3D다각영역
	*/
	static inline const vertexs& XYZS(uint count, int x, int y, int z, ...)
	{
	...
	*/
	...
	\return 색상
	*/
	static inline const color_x888 RGB(const byte r, const byte g, const byte b)
	{
	...
	*/
	...
	{
		const int MaxValue = (r < g)? ((g < b)? b : g) : ((r < b)? b : r);
		const int MinValue = (r > g)? ((g > b)? b : g) : ((r > b)? b : r);
		const fint AddValue = (MaxValue + MinValue) << 16;
		const fint SubValue = (MaxValue - MinValue) << 16;
	...
	*/
	static inline const color_x888 ColorHLSToRGB(const byte h, const byte l, const byte s)
	{
		if(0 < s)
		{
			const fint HValue = (h & 0xFF) << 16, LValue = ((l & 0xFF) << 16) / 255, SValue = ((s & 0xFF) << 16) / 255;
	...
	*/
	...
	public:
		...
		void Init(const int Width, const int Height)
		{
			...
			Color[0].B = 0xFF;
			Color[0].Sum = 0;
			Color[0].Color = _DrawOption::CurColor() = RGB(Color[0].R, Color[0].G, Color[0].B); // 현재색상
			Hue[0] = 0;
		...
		void Sub()
		{
			...
			for(uint i = 0; i < Count[CountFocus]; ++i)
			{
				...
				{
				case drawstyle_clip: _DrawOption::CurClipRect() = Clip[Focus].Rect; // 현재클리핑
	...
	*/
	static stack CLIP(rect r, int hx = 0, int hy = 0)
	{
	...
	\return 스택처리수량(무조건 1), 여러가지 스타일을 +로 복합사용하기 위함
	*/
	static stack HOTSPOT(int hx, int hy)
	{
	...
	\return 스택처리수량(무조건 1), 여러가지 스타일을 +로 복합사용하기 위함
	*/
	static stack MASK(maskoperate operate = maskoperate_default, maskflag flag = maskflag_default)
	{
	...
	\return 스택처리수량(무조건 1), 여러가지 스타일을 +로 복합사용하기 위함
	*/
	static stack ROTATE(int angle1024, bool doFlip = false)
	{
	...
	\return 스택처리수량(무조건 1), 여러가지 스타일을 +로 복합사용하기 위함
	*/
	static stack SCALE(fint hor, fint ver)
	{
	...
	\return 스택처리수량(무조건 1), 여러가지 스타일을 +로 복합사용하기 위함
	*/
	static stack SCALE(double hor, double ver)
	{
	...
	\return 스택처리수량(무조건 1), 여러가지 스타일을 +로 복합사용하기 위함
	*/
	static stack COLOR(byte r, byte g, byte b, uint rate = 100)
	{
		...
		Stack().Color[Focus].Color = _DrawOption::CurColor() = RGB(Stack().Color[Focus].R, Stack().Color[Focus].G, Stack().Color[Focus].B); // 현재색상
		// 스택증가
	...
	\return 스택처리수량(무조건 1), 여러가지 스타일을 +로 복합사용하기 위함
	*/
	static stack COLOR(color_x888 rgb, uint rate = 100)
	{
		return COLOR((rgb >> 16) & 0xFF, (rgb >> 8) & 0xFF, rgb & 0xFF, rate);
	}

	/// @cond SECTION_NAME
	static stack HUE(byte hue)
	{
	...
	*/
	...
	\return 스택처리수량(무조건 1), 여러가지 스타일을 +로 복합사용하기 위함
	*/
	static stack BLEND(drawblend blend)
	{
	...
	\return 스택처리수량(무조건 1), 여러가지 스타일을 +로 복합사용하기 위함
	*/
	static stack FORM(BxDrawForm* form = null, rect r = XYWH(0, 0, size::max, size::max))
	{
		const int Focus = ++Stack().StyleFocus[drawstyle_form];
		BxAssert("BxDraw", Focus < Stack().MAX);
		Stack().Form[Focus].Form = _DrawOption::CurForm() = (form)? form : _BxColorMap::GetClass(); // 현재폼
	...
	\return 스택처리수량(무조건 1), 여러가지 스타일을 +로 복합사용하기 위함
	*/
	static stack MOVE3D(int x, int y, int z)
	{
...
public:
	...
	*/
	BxDraw() : FigureMapWidthFix(32), FigureMapWidthLevel(5)
	{
		...
		int DispFixRot;
		BxCore::System::GetConfigInt("s3e", "DispFixRot", &DispFixRot);
		switch(DispFixRot)
		{
		case 1: dir = dir_rot0; break;
		case 2: dir = dir_rot90; break;
		}
		#endif
		BxCore::Surface::Init(dir);
		// 버튼이벤트초기화
		ButtonLength = 0;
		IsButtonEventFinished = false;
		// 도형버퍼초기화
		FigureMapHeight = 0;
		FigureMap = null;
		// 백버퍼 초기화
	...
	*/
	...
	{
		if(isScreen) return BackBufferWidth;
	...
	*/
	...
	{
		if(isScreen) return BackBufferHeight;
	...
	*/
	void Area(int x, int y, stack count = 0)
	{
		...
		x = (x * BxDrawGlobal::_DrawOption::CurScaleHor()) >> 16;
		y = (y * BxDrawGlobal::_DrawOption::CurScaleVer()) >> 16;
		...
		if(0 < Angle || Turn != turn_0)
		{
			...
			const int TempX = (int)((AngleFlip * x * AngleCos - y * AngleSin) >> 16);
			const int TempY = (int)((AngleFlip * x * AngleSin + y * AngleCos) >> 16);
			x = TempX, y = TempY;
		}
	...
	*/
	void Rectangle(BxDrawGlobal::option opt, rect r, stack count = 0)
	{
		if(!VisibleTest(count)) return;
		#ifdef __BX_OPENGL
		// 조건 이관
		...
		const int Left = ((r.l * ScaleHor) >> 16) + BxDrawGlobal::_DrawOption::CurHotspotX();
		const int Top = ((r.t * ScaleVer) >> 16) + BxDrawGlobal::_DrawOption::CurHotspotY();
		const int Right = ((r.r * ScaleHor) >> 16) + BxDrawGlobal::_DrawOption::CurHotspotX();
		const int Bottom = ((r.b * ScaleVer) >> 16) + BxDrawGlobal::_DrawOption::CurHotspotY();
		rect Rect = {
		...
		if(0 < Rect.r - Rect.l && 0 < Rect.b - Rect.t)
		{
			...
			fint XFocus = (Turn == turn_0 || Turn == turn_90 || Turn == turn_x180 || Turn == turn_x270)? XBegin : (FormArea.w << 16) - 1 - XBegin;
			fint YFocus = (Turn == turn_0 || Turn == turn_x0 || Turn == turn_270 || Turn == turn_x270)? YBegin : (FormArea.h << 16) - 1 - YBegin;
	...
	/// @cond SECTION_NAME
	void Line(rect r, stack count = 0)
	{
		if(!VisibleTest(count)) return;
		#ifdef __BX_OPENGL
		// 모듈
		...
		if(Y1 != Y2)
		{
			...
			for(int y = BxUtilGlobal::Max(BxDrawGlobal::_DrawOption::CurClipRect().t, BeginY), yend = BxUtilGlobal::Min(EndY, BxDrawGlobal::_DrawOption::CurClipRect().b); y < yend; ++y)
			{
				...
				if(xbegin <= xend)
				{
					...
					// 백버퍼카피
					FormMethod(FormClass, FormRect, Dst, xend - xbegin, xbegin << 16, y << 16, 1 << 16, 0, BlendSpriteTable, BlendAlphaTable);
				}
			}
		}
		SubStyle();
	...
	/// @cond SECTION_NAME
	void Ellipse(BxDrawGlobal::option opt, rect r, stack count = 0)
	{
		if(!VisibleTest(count)) return;
		#ifdef __BX_OPENGL
		// 모듈
	...
	\param count : 스타일옵션, 다중설정시 +사용
	*/
	void PolygonSt(BxDrawGlobal::option opt, points p, stack count = 0)
	{
		if(!VisibleTest(count)) return;
		#ifdef __BX_OPENGL
		// 모듈
		...
		{
			const int OX = (p.pt[i].x * Hor + Half) >> 16;
			const int OY = (p.pt[i].y * Ver + Half) >> 16;
			Vertex[i].x = (int)((OX * Cos1 - OY * Sin1 + Half) >> 16);
			Vertex[i].y = (int)((OX * Sin1 + OY * Cos1 + Half) >> 16);
	...
	/// @cond SECTION_NAME
	void PolygonBz(BxDrawGlobal::option opt, points p, stack count = 0)
	{
		if(!VisibleTest(count)) return;
		#ifdef __BX_OPENGL
		// 모듈
	...
	\param count : 스타일옵션, 다중설정시 +사용
	*/
	void Polygon3D(vertexs v, points uv, stack count = 0)
	{
		if(!VisibleTest(count)) return;
		if(!AddStyleByIf(count)) return;
		#ifdef __BX_OPENGL
	...
	/*!
	\brief 백버퍼를 화면에 출력
	\param isInitOnly : 출력없이 버퍼관련 초기화만 수행
	*/
	void Flush(bool isInitOnly = false)
	{
		if(!isInitOnly)
			BxCore::Surface::Flush(BackBufferPtr, sizeof(pixel_dst) * BackBufferWidth, BackBufferHeight, sizeof(pixel_dst) * GetBackBufferPitch());
		// 버튼이벤트초기화
		if(IsButtonEventFinished) ButtonLength = 0;
		else IsButtonEventFinished = true;
		// 백버퍼초기화
		if(BackBufferWidth != BxCore::Surface::GetWidth() || BackBufferHeight != BxCore::Surface::GetHeight())
		{
			BackBufferWidth = BxCore::Surface::GetWidth();
			BackBufferHeight = BxCore::Surface::GetHeight();
			#ifndef __BX_OPENGL
			BxDelete_Array(BackBufferPtr);
		...
		// 스택초기화
		BxDrawGlobal::Stack().CountCurrently = 0;
		SubStyle(true);
	}
	...
	*/
	bool AddStyleByIf(stack count, string buttonName = null)
	{
		AddStyle(count);
	...
	*/
	void SubStyle(bool doAll = false)
	{
		BxAssert("BxDraw", BxDrawGlobal::Stack().CountCurrently == 0);
...
public:
	...
	public:
		...
		rect Field;
	public:
		_ButtonEvent() : Name(null) {}
		virtual ~_ButtonEvent() {BxDelete_Array(Name);}
	public:
		void SetEvent(string name, rect field)
		{
			const int Length = BxUtilGlobal::StrLen(name);
			BxDelete_Array(Name);
			Name = BxNew_Array(char, Length + 1);
...
protected:
	...
	/// @cond SECTION_NAME
	inline bool VisibleTest(stack count)
	{
