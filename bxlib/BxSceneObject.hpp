...
public:
	BxSceneObject() {Init(-1);}
	virtual ~BxSceneObject() {Quit(-1);}
	void Init(int index)
	{
...
public:
	...
	{
		XMLParser.DefineTag(TAG_SCENEOBJECT, "SCENEOBJECT");
		...
		XMLParser.DefineAttb(TAG_SCENEOBJECT, ATTB_SCENEOBJECT_VERSION, "VERSION", "");
		...
		if(XMLParser.ReadBegin(FileName))
		{
			...
			while(State != BxXml::ENDOFFILE && State != BxXml::ERROR)
			{
				...
				if(State == BxXml::OPEN)
				{
					switch(TAG)
					{
					case TAG_SCENEOBJECT:
						if(CurSection == NoSection)
							CurSection = InSceneObject;
						continue;
					case TAG_LAYOUT:
						if(CurSection == InSceneObject)
						{
							CurSection = InLayout;
							Layout.Insert(LAST);
						}
...
protected:
	/// @cond SECTION_NAME
	enum Section {NoSection, InSceneObject, InLayout, InContent};
	...
	public:
		...
		const int GetValue()
		{
			BxAssert("BxSceneObject<값보관용 엘리먼트가 아닙니다>", Type == TypeValue);
			return Value;
		}
	...
	public:
		void SetStep(ElementStep StepOrder)
		{
			switch(StepOrder)
			{
			case StepNamed: BxAssert("BxSceneObject<호출단계가 잘못되었습니다>", Step == StepAlloced); break;
			case StepPreProcessed: BxAssert("BxSceneObject<호출단계가 잘못되었습니다>", Step == StepNamed); break;
			case StepProcessed: BxAssert("BxSceneObject<호출단계가 잘못되었습니다>", Step == StepPreProcessed); break;
			case StepLinked: BxAssert("BxSceneObject<호출단계가 잘못되었습니다>", Step == StepProcessed); break;
			}
			Step = StepOrder;
		}
	public:
		virtual bool IsValidFill() {return (Type != TypeValue);}
		virtual void PreProcessing(BxVarMap<Element*>* Map, BxVar<Element>* Elements) {SetStep(StepPreProcessed);}
		virtual void Processing(BxVarMap<Element*>* Map, BxVar<Element>* Elements) {SetStep(StepProcessed);}
		virtual void Linking(const BxVarMap<Element*>& Map) {SetStep(StepLinked);}
		virtual void Filling(BxDraw& Draw, rect Rect) {}
		virtual rect Rendering(const BxVarMap<Element*>& Map, BxDraw& Draw, rect Rect, rect _out_ InRect) {return rect::zero();}
		virtual bool OnEvent(const sysevent& SysEvent) {return false;}
	protected:
		bool DownloadCache(string url, BxWebContent _out_ Content, int _out_ FileSize, string _out_ FilePath, bool DoLoad)
		{
			BxString QueryArg("subject=%s&file=%s&version=%d", null, (string) BxSceneObject::Subject(), url, BxSceneObject::Version());
			BxWebContent Web;
			if(Web.Query(BxSceneObject::Domain(), 80, "update/res_download_cdn.aspx", QueryArg))
			{
				...
				if(QueryResults.Length() == 3 && BxUtilGlobal::AtoI(QueryResults[0]) == 1)
				{
					...
					if(Content.Cache(BxSceneObject::Domain(), 80, FilePath, FileSize, DoLoad))
					{
						BxFree(QueryResult);
						return true;
					}
	...
	public:
		...
		void LinkingTree(const BxVarMap<Element*>& Map)
		{
			if(Parent) Linking(Map);
			for(int i = 0; i < Child.Length(); ++i)
				Child[i].LinkingTree(Map);
		}
		rect RenderingTree(const BxVarMap<Element*>& Map, BxDraw& Draw, rect Rect)
		{
			rect OutRect = Rect, InRect = Rect;
			if(Draw.AddStyleByIf(CLIP(Rect, -Rect.l, -Rect.t)))
			{
	...
	public:
		...
		class Edge : public Element
		{
			...
			string Colors;
		public:
			BxVar<Element*> _ref_ ColorValues;
		public:
		...
		public:
			virtual void Linking(const BxVarMap<Element*>& Map)
			{
				SetStep(StepLinked);
				BxVar<BxString> ColorNames;
				BxString::ParseDivision(Colors, ColorNames);
				BxAssert("BxSceneObject<연결작업중 유효한 엘리먼트가 하나도 없습니다>", ColorNames.Length());
				for(int i = 0; i < ColorNames.Length(); ++i)
				{
					BxAssert("BxSceneObject<연결작업에 실패했습니다>", Map.Access(ColorNames[i]) && *Map.Access(ColorNames[i]));
					BxAssert("BxSceneObject<연결작업중 타입이 일치하지 않습니다>", (*Map.Access(ColorNames[i]))->GetType() == TypeColor);
		...
		public:
			virtual void Processing(BxVarMap<Element*>* Map, BxVar<Element>* Elements)
			{
				...
				if(DownloadCache(Url, Content, FileSize, FilePath, true))
				{
					...
					CacheImage.Load(Content.GetBytes(), FileType);
					CacheImage.SetHotspot(0, 0);
				}
				// 가로길이 추가등록
				BxString WidthName("%s.width", null, (string) Name);
				BxAssert("BxSceneObject<중복된 컨텐트명입니다>", !Map->Access(WidthName));
				...
				BxAssert("BxSceneObject<중복된 컨텐트명입니다>", !Map->Access(HeightName));
			...
			virtual void Filling(BxDraw& Draw, rect Rect)
			{
				if(CacheImage.IsExist() && Draw.AddStyleByIf(CLIP(Rect)))
				{
		...
		class Sprite : public Element
		{
			...
			int Delay;
		public:
			BxVar<Element*> _ref_ ImageValues;
		public:
		...
		public:
			virtual void Linking(const BxVarMap<Element*>& Map)
			{
				...
				BxAssert("BxSceneObject<연결작업중 유효한 엘리먼트가 하나도 없습니다>", ImageNames.Length());
				for(int i = 0; i < ImageNames.Length(); ++i)
				{
					BxAssert("BxSceneObject<연결작업에 실패했습니다>", Map.Access(ImageNames[i]) && *Map.Access(ImageNames[i]));
					BxAssert("BxSceneObject<연결작업중 타입이 일치하지 않습니다>", (*Map.Access(ImageNames[i]))->GetType() == TypeImage);
		...
		public:
			...
			virtual void PreProcessing(BxVarMap<Element*>* Map, BxVar<Element>* Elements)
			{
		...
		public:
			...
			void AdjustFormatNoList(BxVarMap<Element*>* Map, BxVar<Element>* Elements, ...)
			{
				va_list List;
			...
			void AdjustFormat(bool dosprint, va_list list, BxVarMap<Element*>* Map, BxVar<Element>* Elements)
			{
				BxFree(FormatGen);
				if(dosprint)
				{
					const int FormatGenLen = BxCore::Util::VsnPrintf(null, 0, Format, list);
					BxAssert("BxSceneObject<FormatGen의 예상길이를 얻는데 실패하였습니다>", 0 <= FormatGenLen);
					FormatGen = (string) BxAlloc(FormatGenLen + 1);
		...
		public:
			...
			{
				SetStep(StepProcessed);
				BxAssert("BxSceneObject<연결작업에 실패했습니다>", Map->Access(Font) && *Map->Access(Font));
				BxAssert("BxSceneObject<연결작업중 타입이 일치하지 않습니다>", (*Map->Access(Font))->GetType() == TypeFont);
			...
			{
				SetStep(StepLinked);
				BxAssert("BxSceneObject<연결작업에 실패했습니다>", Map.Access(Color) && *Map.Access(Color));
				BxAssert("BxSceneObject<연결작업중 타입이 일치하지 않습니다>", (*Map.Access(Color))->GetType() == TypeColor);
				ColorValue = (Content::Color*) *Map.Access(Color);
				BxAssert("BxSceneObject<연결작업에 실패했습니다>", Map.Access(Vector) && *Map.Access(Vector));
				VectorValue = (ReserveCode) (*Map.Access(Vector))->GetValue();
				BxAssert("BxSceneObject<연결작업에 실패했습니다>", Map.Access(Line) && *Map.Access(Line));
			...
			virtual void Filling(BxDraw& Draw, rect Rect)
			{
				...
				if(0 < RectWidth && 0 < RectHeight && FontValue->GetCacheFont() && Draw.AddStyleByIf(CLIP(Rect)))
				{
					switch(VectorValue)
					{
					default:
		...
		public:
			BxVar<Element*> _ref_ ImageValues;
			BxVar<Element*> _ref_ EdgeValues;
		public:
		...
		public:
			virtual void Linking(const BxVarMap<Element*>& Map)
			{
				...
				{
					BxAssert("BxSceneObject<연결작업에 실패했습니다>", Map.Access(EdgeNames[i]) && *Map.Access(EdgeNames[i]));
					BxAssert("BxSceneObject<연결작업중 타입이 일치하지 않습니다>", (*Map.Access(EdgeNames[i]))->GetType() == TypeEdge);
			...
			virtual bool OnEvent(const sysevent& SysEvent)
			{
				...
				{
					if(SysEvent.touch.type == systouchtype_up && IsClickedGen && BxSceneObject::Callback() &&
						!(SysEvent.touch.x < EventAreaGen.l || SysEvent.touch.y < EventAreaGen.t ||
						EventAreaGen.r <= SysEvent.touch.x || EventAreaGen.b <= SysEvent.touch.y))
						BxSceneObject::Callback()(true, Event, (void*) Link);
					IsClickedGen = false;
				}
				return IsClickedGen;
			}
		...
		public:
			...
			{
				SetStep(StepLinked);
				BxAssert("BxSceneObject<연결작업에 실패했습니다>", Map.Access(Text) && *Map.Access(Text));
				BxAssert("BxSceneObject<연결작업중 타입이 일치하지 않습니다>", (*Map.Access(Text))->GetType() == TypeText);
	...
	public:
		void PreProcessing(BxVarMap<Element*>& Map)
		{
			for(int i = 0; i < Elements.Length(); ++i)
				Elements[i].PreProcessing(&Map, &Elements);
		}
		void Processing(BxVarMap<Element*>& Map)
		{
			for(int i = 0; i < Elements.Length(); ++i)
				Elements[i].Processing(&Map, &Elements);
		}
		void Linking(const BxVarMap<Element*>& Map)
		{
			for(int i = 0; i < Elements.Length(); ++i)
				Elements[i].Linking(Map);
		}
		void AddNull(BxVarMap<Element*>& Map, string Name)
		{
			BxAssert("BxSceneObject<중복된 컨텐트명입니다>", !Map.Access(Name));
			Element* NewData = BxNew(Element);
			Elements.Insert(LAST, Map[Name] = NewData);
			NewData->SetStep(StepNamed);
		}
		void AddValue(BxVarMap<Element*>& Map, string Name, int Value)
		{
			BxAssert("BxSceneObject<중복된 컨텐트명입니다>", !Map.Access(Name));
		...
		void AddColor(BxVarMap<Element*>& Map, string Name, string Argb)
		{
			BxAssert("BxSceneObject<중복된 컨텐트명입니다>", !Map.Access(Name));
		...
		void AddEdge(BxVarMap<Element*>& Map, string Name, string Colors)
		{
			BxAssert("BxSceneObject<중복된 컨텐트명입니다>", !Map.Access(Name));
		...
		void AddImage(BxVarMap<Element*>& Map, string Name, string Url)
		{
			BxAssert("BxSceneObject<중복된 컨텐트명입니다>", !Map.Access(Name));
		...
		void AddSprite(BxVarMap<Element*>& Map, string Name, string Images, string Delay)
		{
			BxAssert("BxSceneObject<중복된 컨텐트명입니다>", !Map.Access(Name));
		...
		void AddFont(BxVarMap<Element*>& Map, string Name, string Size, string Url)
		{
			BxAssert("BxSceneObject<중복된 컨텐트명입니다>", !Map.Access(Name));
		...
		void AddText(BxVarMap<Element*>& Map, string Name, string Format, string Font, string Color, string Vector, string Line)
		{
			BxAssert("BxSceneObject<중복된 컨텐트명입니다>", !Map.Access(Name));
		...
		void AddButton(BxVarMap<Element*>& Map, string Name, string Images, string Edges, string Event, string Link)
		{
			BxAssert("BxSceneObject<중복된 컨텐트명입니다>", !Map.Access(Name));
		...
		void AddEditBox(BxVarMap<Element*>& Map, string Name, string Text, string Colors)
		{
			BxAssert("BxSceneObject<중복된 컨텐트명입니다>", !Map.Access(Name));
	...
	public:
		...
		protected:
			...
			{
				SetStep(StepLinked);
				BxAssert("BxSceneObject<연결작업에 실패했습니다>", Map.Access(Pos) && *Map.Access(Pos));
				PosValue = (ReserveCode) (*Map.Access(Pos))->GetValue();
				BxAssert("BxSceneObject<연결작업에 실패했습니다>", Map.Access(Fill) && *Map.Access(Fill));
				BxAssert("BxSceneObject<FILL용 엘리먼트가 아닙니다>", (*Map.Access(Fill))->IsValidFill());
				FillValue = *Map.Access(Fill);
			}
			virtual rect Rendering(const BxVarMap<Element*>& Map, BxDraw& Draw, rect Rect, rect _out_ InRect)
			{
				InRect = Rect;
				rect OutRect = Rect;
				const int RectWidth = Rect.r - Rect.l;
		...
		protected:
			...
			{
				SetStep(StepLinked);
				BxAssert("BxSceneObject<outline/size에서는 백분율이 사용될 수 없습니다>", Size[BxUtilGlobal::StrLen(Size) - 1] != '%');
				BxAssert("BxSceneObject<연결작업에 실패했습니다>", Map.Access(Fill) && *Map.Access(Fill));
				BxAssert("BxSceneObject<FILL용 엘리먼트가 아닙니다>", (*Map.Access(Fill))->IsValidFill());
				FillValue = *Map.Access(Fill);
			}
		...
		protected:
			...
			{
				SetStep(StepLinked);
				BxAssert("BxSceneObject<연결작업에 실패했습니다>", Map.Access(PosX) && *Map.Access(PosX));
				PosXValue = (ReserveCode) (*Map.Access(PosX))->GetValue();
				BxAssert("BxSceneObject<연결작업에 실패했습니다>", Map.Access(PosY) && *Map.Access(PosY));
				PosYValue = (ReserveCode) (*Map.Access(PosY))->GetValue();
				BxAssert("BxSceneObject<연결작업에 실패했습니다>", Map.Access(Scroll) && *Map.Access(Scroll));
	...
	public:
		...
		void AddSplit(BxVarMap<Element*>& Map, string Name, string Pos, string Size, string Fill)
		{
		...
		void AddOutline(BxVarMap<Element*>& Map, string Name, string Size, string Fill)
		{
		...
		void AddExtra(BxVarMap<Element*>& Map, string Name, string Width, string Height, string Fill)
		{
		...
		void AddView(BxVarMap<Element*>& Map, string Name, string PosX, string PosY, string Width, string Height, string Scroll, string Fill)
		{
		...
		void Rendering(const BxVarMap<Element*>& Map, BxDraw& Draw, rect Rect)
		{
