...
#define BxKeyEvent(NAME) (BxKeyword::Access<BxKeyword::Event>::ByTemplate<class _BXKW_##NAME>(#NAME).LetString())
#define BxKeyCheck(NAME) (BxKeyword::Access<BxKeyword::Check>::ByTemplate<class _BXKW_##NAME>(#NAME).LetBool())
#define BxKeyRadio_Get(NAME) (BxKeyword::Access<BxKeyword::Radio>::ByTemplate<class _BXKW_##NAME>(#NAME).GetConst())
#define BxKeyRadio_On(NAME) (BxKeyword::Access<BxKeyword::Radio>::ByTemplate<class _BXKW_##NAME>(#NAME).SetOn())
#define BxKeyRadio_Off(NAME) (BxKeyword::Access<BxKeyword::Radio>::ByTemplate<class _BXKW_##NAME>(#NAME).SetOff())
#define BxKeyNumber(NAME) (BxKeyword::Access<BxKeyword::Number>::ByTemplate<class _BXKW_##NAME>(#NAME).LetInt())
#define BxKeyString(NAME) (BxKeyword::Access<BxKeyword::String>::ByTemplate<class _BXKW_##NAME>(#NAME).LetString())
...
class BxKeyword
{
	...
	{
		global_data BxVar<BxKeyword> Pool;
		return Pool;
	}
...
public:
	...
	global_func bool Load(string filename, callback_event method = null)
	{
		...
		if(KeywordFile)
		{
			string_rw KeywordBuffer = (string_rw) BxAlloc(BxCore::File::GetSize(KeywordFile) + 1);
			BxCore::File::Read(KeywordFile, KeywordBuffer, BxCore::File::GetSize(KeywordFile));
			KeywordBuffer[BxCore::File::GetSize(KeywordFile)] = '\0';
			BxCore::File::Close(KeywordFile);
			LoadScript(KeywordBuffer, method);
			BxFree(KeywordBuffer);
			return true;
		}
		return false;
	}
	...
	{
		global_data string Command[7] = {"keyword_begin", "keyword_end", "event", "check", "radio", "number", "string"};
		global_data const int CommandLength[7] = {BxUtilGlobal::StrLen(Command[0]), BxUtilGlobal::StrLen(Command[1]), BxUtilGlobal::StrLen(Command[2]),
			BxUtilGlobal::StrLen(Command[3]), BxUtilGlobal::StrLen(Command[4]), BxUtilGlobal::StrLen(Command[5]), BxUtilGlobal::StrLen(Command[6])};
		bool IsBegin = false;
		...
		for(int i = 0; buffer[i] != '\0'; ++i)
		{
			if(!IsWaitReturn)
			{
				if(buffer[i] == '(')
				{
					for(int j = 0, jend = (IsBegin)? 7 : 1; j < jend; ++j)
					{
						if(i < CommandLength[j]) continue;
						bool IsMatch = true;
						for(int k = 1; k <= CommandLength[j]; ++k)
							if(buffer[i - k] != Command[j][CommandLength[j] - k])
							{
								IsMatch = false;
								break;
							}
							...
							else
							{
								...
								if(buffer[i] == ')')
								{
									...
									case 2: case 6:
										...
										break;
									default:
										while(buffer[i] != '\0' && (buffer[++i] == ' ' || buffer[i] == '\t')); Value = buffer[i];
									...
									switch(j)
									{
									case 2: MakeEvent(Name, Value, method); break;
									case 3: Access<Check>::BySearch(Name, true)->LetBool() = (Value.GetLength() || Value[0] == 't')? true : false; break;
									case 4: MakeRadio(Name, (Value.GetLength())? BxUtilGlobal::AtoI(Value) : -1); break;
									case 5: Access<Number>::BySearch(Name, true)->LetInt() = (Value.GetLength())? BxUtilGlobal::AtoI(Value) : 0; break;
									case 6: Access<String>::BySearch(Name, true)->LetString() = Value; break;
									}
								}
								break;
							}
	...
	global_func void Save(string filename)
	{
		...
		if(KeywordFile)
		{
			BxCore::File::WriteString(KeywordFile, "keyword_begin()\r\n");
			// event
			{
	...
	global_func void MakeRadio(string name, int value)
	{
